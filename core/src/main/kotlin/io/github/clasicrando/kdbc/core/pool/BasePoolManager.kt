...
/**
 * Manager of [Connection] and [ConnectionPool] instances for a given database vendor. This type
 * holds a collection of [ConnectionPool] instances per connect options provided. Then each
 * vendor's [Connection] will have a static method that calls this [acquireConnection] to fetch the
 * connect option's [ConnectionPool] and acquire a [Connection] from that pool. If there is no
...
abstract class BasePoolManager<O : Any, C : Connection> {
     ...
     * [ConnectionPool.acquire] on the [ConnectionPool] associated with the [connectOptions]
