...
class TestValueTypeDescription {
    ...
    fun `encode should accept Wrapper when querying postgresql`(): Unit =
        runBlocking {
    ...
    @Test
    @Timeout(value = DEFAULT_KDBC_TEST_TIMEOUT)
    fun `decode should return Wrapper when querying postgresql text`(): Unit =
        runBlocking {
            val wrapper = Wrapper("inner")
