            ...
            pool.useConnection {
                ...
                val copyResult =
                    IOUtils.source(testFilePath).buffered().use { source ->
                        it.copyIn(copyInStatement, source)
                    }
                assertEquals(ROW_COUNT_LONG, copyResult.rowsAffected)
                assertEquals("COPY $ROW_COUNT", copyResult.message)
                val count = query("SELECT COUNT(*) FROM public.copy_in_test").fetchScalar<Long>(it)
            ...
            val copyResult =
                it.copyIn(
                    ...
                    (1..ROW_COUNT).asFlow().map { i ->
                        CopyInTestRow(id = i, textValue = "$i Value")
                    },
                )
            assertEquals(ROW_COUNT_LONG, copyResult.rowsAffected)
            assertEquals("COPY $ROW_COUNT", copyResult.message)
    ...
    fun `copyOut should write all rows from table when csv`(): Unit = runBlocking {
        ...
        try {
            pool.useConnection {
                ...
                val copyOutStatement =
                    CopyStatement.TableToCsv(schemaName = "public", tableName = "copy_out_test")
                IOUtils.sink(path).buffered().use { sink -> it.copyOut(copyOutStatement, sink) }
