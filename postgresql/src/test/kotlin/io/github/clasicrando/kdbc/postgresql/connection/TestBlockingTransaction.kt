...
class TestBlockingTransaction {
    ...
    @Test
    fun `transaction commits when successful SQL statement`() {
    ...
    @Test
    fun `transaction rolls back when failed SQL statement`() {
        pool.useConnection { conn ->
            val countBefore = conn.createQuery("SELECT COUNT(0) FROM public.$TABLE_NAME")
                .fetchScalar<Long>()
            assertEquals(0L, countBefore)
    ...
    companion object {
        ...
        """
        private val pool = PgConnectionHelper.defaultBlockingPool()
