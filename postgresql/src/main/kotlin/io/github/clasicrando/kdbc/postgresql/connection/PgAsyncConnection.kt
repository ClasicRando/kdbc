...
import io.github.clasicrando.kdbc.postgresql.query.PgAsyncQuery
...
/**
 * [AsyncConnection] object for a Postgresql database. A new instance cannot be created but
 * rather the [io.github.clasicrando.kdbc.postgresql.Postgres.asyncConnection] method should
 * be called to receive a new [PgAsyncConnection] ready for user usage. This method will use
 ...
 */
class PgAsyncConnection internal constructor(
    ...
    override fun createQuery(query: String): AsyncQuery = PgAsyncQuery(this, query)

    override fun createPreparedQuery(query: String): AsyncPreparedQuery {
        return PgAsyncPreparedQuery(this, query)
    }

    override fun createPreparedQueryBatch(): AsyncPreparedQueryBatch {
        return PgAsyncPreparedQueryBatch(this)
    }
    ...
    override suspend fun close() {
        if (!pool.giveBack(this@PgAsyncConnection)) {
            dispose()
        }
    }
    ...
    companion object {
        ...
        internal suspend fun connect(
            ...
            var connection: PgAsyncConnection? = null
            try {
                connection = PgAsyncConnection(connectOptions, stream, pool)
