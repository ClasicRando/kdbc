...
internal object LocalTimeTypeDescription :
    ...
    override fun encode(value: LocalTime, buffer: Sink) {
        val microSeconds =
            value.toNanosecondOfDay().toDuration(DurationUnit.NANOSECONDS).inWholeMicroseconds
 ...
 */
internal object PgTimeTzTypeDescription :
    PgTypeDescription<PgTimeTz>(dbType = PgType.Timetz, kType = typeOf<PgTimeTz>()) {
    /**
     ...
     */
    override fun encode(value: PgTimeTz, buffer: Sink) {
     ...
     * Read a [Long] value as the microseconds from the start of the data and use that to construct
     * a [LocalTime] by multiplying the [Long] value by 1000 and calling
     * [LocalTime.fromNanosecondOfDay]. Also, read an [Int] and negate the value before constructing
     * a [UtcOffset] with that total number of seconds as the offset. The value must be negated
     * since postgres treats a positive offset as west while [UtcOffset] treats east as a positive
     * offset. Both values are then passed to the [PgTimeTz] constructor.
     *
     * [pg source
 ...
 */
internal object JLocalTimeTypeDescription :
    PgTypeDescription<java.time.LocalTime>(
        dbType = PgType.Time,
     ...
     * code](https://github.com/postgres/postgres/blob/874d817baa160ca7e68bee6ccc9fc1848c56e750/src/backend/utils/adt/date.c#L1521)
     */
    override fun encode(value: java.time.LocalTime, buffer: Sink) {
        val microSeconds =
