...
internal abstract class ArrayTypeDescription<T : Any>(
     ...
     *
     * [pg source
     * code](https://github.com/postgres/postgres/blob/d57b7cc3338e9d9aa1d7c5da1b25a17c5a72dcce/src/backend/utils/adt/arrayfuncs.c#L1549)
     *
    ...
    override fun decodeBytes(value: PgValue.Binary): List<T?> {
        ...
        checkOrColumnDecodeError(check = dimensions == 1, kType = kType, type = value.typeData) {
        ...
        checkOrColumnDecodeError(check = lowerBound == 1, kType = kType, type = value.typeData) {
    ...
    /**
     * Message is a text representation of the array (also called an array literal). Must be wrapped
     * in curly braces and each item is separated by a comma. For details see [ArrayLiteralParser].
     * With all the items in text format, push each [String] item into a [PgValue.Text] and decode
     * into the resulting [List].
     *
     * [pg source
     * code](https://github.com/postgres/postgres/blob/d57b7cc3338e9d9aa1d7c5da1b25a17c5a72dcce/src/backend/utils/adt/arrayfuncs.c#L1017)
     *
     * @throws io.github.clasicrando.kdbc.core.column.columnDecodeError if the decode operation
     *   fails (reason supplied in [Exception])
     */
