...
public sealed class SqlState(public val errorCode: String, public val conditionName: String) {
    public data object SuccessfulCompletion :
        SqlState(SUCCESSFUL_COMPLETION_CODE, "successful_completion")

    public data object Warning : SqlState(WARNING_CODE, "warning")

    public data object DynamicResultSetsReturned :
        SqlState(DYNAMIC_RESULT_SETS_RETURNED_CODE, "dynamic_result_sets_returned")

    public data object ImplicitZeroBitPadding :
        SqlState(IMPLICIT_ZERO_BIT_PADDING_CODE, "implicit_zero_bit_padding")

    public data object NullValueEliminatedInSetFunction :
        SqlState(
            NULL_VALUE_ELIMINATED_IN_SET_FUNCTION_CODE,
            "null_value_eliminated_in_set_function",
        )

    public data object PrivilegeNotGranted :
        SqlState(PRIVILEGE_NOTE_GRANTED, "privilege_not_granted")

    public data object PrivilegeNotRevoked :
        SqlState(PRIVILEGE_NOT_REVOKED, "privilege_not_revoked")

    public data object StringDataRightTruncation :
        SqlState(STRING_DATA_RIGHT_TRUNCATION, "string_data_right_truncation")

    public data object DeprecatedFeature : SqlState(DEPRECATED_FEATURE, "deprecated_feature")

    public data object NoData : SqlState(NO_DATA, "no_data")

    public data object NoAdditionalDynamicResultSetsReturned :
        SqlState(
            NO_ADDITIONAL_DYNAMIC_RESULT_SETS_RETURNED,
            "no_additional_dynamic_result_sets_returned",
        )

    public data object SqlStatementNotYetComplete :
        SqlState(SQL_STATEMENT_NOT_YET_COMPLETE, "sql_statement_not_yet_complete")

    public data object ConnectionException : SqlState(CONNECTION_EXCEPTION, "connection_exception")

    public data object ConnectionDoesNotExist :
        SqlState(CONNECTION_DOES_NOT_EXIST, "connection_does_not_exist")

    public data object ConnectionFailure : SqlState(CONNECTION_FAILURE, "connection_failure")

    public data object SqlClientUnableToEstablishSqlConnection :
        SqlState(
            SQL_CLIENT_UNABLE_TO_ESTABLISH_SQL_CONNECTION,
            "sqlclient_unable_to_establish_sqlconnection",
        )

    public data object SqlServerRejectedEstablishmentOfSqlConnection :
        SqlState(
            SQL_SERVER_REJECTED_ESTABLISHMENT_OF_SQL_CONNECTION,
            "sqlserver_rejected_establishment_of_sqlconnection",
        )

    public data object TransactionResolutionUnknown :
        SqlState(TRANSACTION_RESOLUTION_UNKNOWN, "transaction_resolution_unknown")

    public data object ProtocolViolation : SqlState(PROTOCOL_VIOLATION, "protocol_violation")

    public data object TriggeredActionException :
        SqlState(TRIGGERED_ACTION_EXCEPTION, "triggered_action_exception")

    public data object FeatureNotSupported :
        SqlState(FEATURE_NOT_SUPPORTED, "feature_not_supported")

    public data object InvalidTransactionInitiation :
        SqlState(INVALID_TRANSACTION_INITIATION, "invalid_transaction_initiation")

    public data object LocatorException : SqlState(LOCATOR_EXCEPTION, "locator_exception")

    public data object InvalidLocatorSpecification :
        SqlState(INVALID_LOCATOR_SPECIFICATION, "invalid_locator_specification")

    public data object InvalidGrantor : SqlState(INVALID_GRANTOR, "invalid_grantor")

    public data object InvalidGrantOperation :
        SqlState(INVALID_GRANT_OPERATION, "invalid_grant_operation")

    public data object InvalidRoleSpecification :
        SqlState(INVALID_ROLE_SPECIFICATION, "invalid_role_specification")

    public data object DiagnosticsException :
        SqlState(DIAGNOSTICS_EXCEPTION, "diagnostics_exception")

    public data object StackedDiagnosticsAccessedWithoutActiveHandler :
        SqlState(
            STACKED_DIAGNOSTICS_ACCESSED_WITHOUT_ACTIVE_HANDLER,
            "stacked_diagnostics_accessed_without_active_handler",
        )

    public data object CaseNotFound : SqlState(CASE_NOT_FOUND, "case_not_found")

    public data object CardinalityViolation :
        SqlState(CARDINALITY_VIOLATION, "cardinality_violation")

    public data object DataException : SqlState(DATA_EXCEPTION, "data_exception")

    public data object ArraySubscriptError :
        SqlState(ARRAY_SUBSCRIPT_ERROR, "array_subscript_error")

    public data object CharacterNotInRepertoire :
        SqlState(CHARACTER_NOT_IN_REPERTOIRE, "character_not_in_repertoire")

    public data object DatetimeFieldOverflow :
        SqlState(DATETIME_FIELD_OVERFLOW, "datetime_field_overflow")

    public data object DivisionByZero : SqlState(DIVISION_BY_ZERO, "division_by_zero")

    public data object ErrorInAssignment : SqlState(ERROR_IN_ASSIGNMENT, "error_in_assignment")

    public data object EscapeCharacterConflict :
        SqlState(ESCAPE_CHARACTER_CONFLICT, "escape_character_conflict")

    public data object IndicatorOverflow : SqlState(INDICATOR_OVERFLOW, "indicator_overflow")

    public data object IntervalFieldOverflow :
        SqlState(INTERVAL_FIELD_OVERFLOW, "interval_field_overflow")

    public data object InvalidArgumentForLogarithm :
        SqlState(INVALID_ARGUMENT_FOR_LOGARITHM, "invalid_argument_for_logarithm")

    public data object InvalidArgumentForNtileFunction :
        SqlState(INVALID_ARGUMENT_FOR_NTILE_FUNCTION, "invalid_argument_for_ntile_function")

    public data object InvalidArgumentForNthValueFunction :
        SqlState(
            INVALID_ARGUMENT_FOR_NTH_VALUE_FUNCTION,
            "invalid_argument_for_nth_value_function",
        )

    public data object InvalidArgumentForPowerFunction :
        SqlState(INVALID_ARGUMENT_FOR_POWER_FUNCTION, "invalid_argument_for_power_function")

    public data object InvalidArgumentForWidthBucketFunction :
        SqlState(
            INVALID_ARGUMENT_FOR_WIDTH_BUCKET_FUNCTION,
            "invalid_argument_for_width_bucket_function",
        )

    public data object InvalidCharacterValueForCast :
        SqlState(INVALID_CHARACTER_VALUE_FOR_CAST, "invalid_character_value_for_cast")

    public data object InvalidDatetimeFormat :
        SqlState(INVALID_DATETIME_FORMAT, "invalid_datetime_format")

    public data object InvalidEscapeCharacter :
        SqlState(INVALID_ESCAPE_CHARACTER, "invalid_escape_character")

    public data object InvalidEscapeOctet : SqlState(INVALID_ESCAPE_OCTET, "invalid_escape_octet")

    public data object InvalidEscapeSequence :
        SqlState(INVALID_ESCAPE_SEQUENCE, "invalid_escape_sequence")

    public data object NonstandardUseOfEscapeCharacter :
        SqlState(NONSTANDARD_USE_OF_ESCAPE_CHARACTER, "nonstandard_use_of_escape_character")

    public data object InvalidIndicatorParameterValue :
        SqlState(INVALID_INDICATOR_PARAMETER_VALUE, "invalid_indicator_parameter_value")

    public data object InvalidParameterValue :
        SqlState(INVALID_PARAMETER_VALUE, "invalid_parameter_value")

    public data object InvalidPrecedingOrFollowingSize :
        SqlState(INVALID_PRECEDING_OR_FOLLOWING_SIZE, "invalid_preceding_or_following_size")

    public data object InvalidRegularExpression :
        SqlState(INVALID_REGULAR_EXPRESSION, "invalid_regular_expression")

    public data object InvalidRowCountInLimitClause :
        SqlState(INVALID_ROW_COUNT_IN_LIMIT_CLAUSE, "invalid_row_count_in_limit_clause")

    public data object InvalidRowCountInResultOffsetClause :
        SqlState(
            INVALID_ROW_COUNT_IN_RESULT_OFFSET_CLAUSE,
            "invalid_row_count_in_result_offset_clause",
        )

    public data object InvalidTableSampleArgument :
        SqlState(INVALID_TABLE_SAMPLE_ARGUMENT, "invalid_tablesample_argument")

    public data object InvalidTableSampleRepeat :
        SqlState(INVALID_TABLE_SAMPLE_REPEAT, "invalid_tablesample_repeat")

    public data object InvalidTimeZoneDisplacementValue :
        SqlState(INVALID_TIME_ZONE_DISPLACEMENT_VALUE, "invalid_time_zone_displacement_value")

    public data object InvalidUseOfEscapeCharacter :
        SqlState(INVALID_USE_OF_ESCAPE_CHARACTER, "invalid_use_of_escape_character")

    public data object MostSpecificTypeMismatch :
        SqlState(MOST_SPECIFIC_TYPE_MISMATCH, "most_specific_type_mismatch")

    public data object NullValueNotAllowed :
        SqlState(NULL_VALUE_NOT_ALLOWED, "null_value_not_allowed")

    public data object NullValueNoIndicatorParameter :
        SqlState(NULL_VALUE_NO_INDICATOR_PARAMETER, "null_value_no_indicator_parameter")

    public data object NumericValueOutOfRange :
        SqlState(NUMERIC_VALUE_OUT_OF_RANGE, "numeric_value_out_of_range")

    public data object SequenceGeneratorLimitExceeded :
        SqlState(SEQUENCE_GENERATOR_LIMIT_EXCEEDED, "sequence_generator_limit_exceeded")

    public data object StringDataLengthMismatch :
        SqlState(STRING_DATA_LENGTH_MISMATCH, "string_data_length_mismatch")

    public data object StringDataRightTruncation2 :
        SqlState(STRING_DATA_RIGHT_TRUNCATION2, "string_data_right_truncation")

    public data object SubstringError : SqlState(SUBSTRING_ERROR, "substring_error")

    public data object TrimError : SqlState(TRIM_ERROR, "trim_error")

    public data object UnterminatedCString :
        SqlState(UNTERMINATED_C_STRING, "unterminated_c_string")

    public data object ZeroLengthCharacterString :
        SqlState(ZERO_LENGTH_CHARACTER_STRING, "zero_length_character_string")

    public data object FloatingPointException :
        SqlState(FLOATING_POINT_EXCEPTION, "floating_point_exception")

    public data object InvalidTextRepresentation :
        SqlState(INVALID_TEXT_REPRESENTATION, "invalid_text_representation")

    public data object InvalidBinaryRepresentation :
        SqlState(INVALID_BINARY_REPRESENTATION, "invalid_binary_representation")

    public data object BadCopyFileFormat : SqlState(BAD_COPY_FILE_FORMAT, "bad_copy_file_format")

    public data object UntranslatableCharacter :
        SqlState(UNTRANSLATABLE_CHARACTER, "untranslatable_character")

    public data object NotAnXmlDocument : SqlState(NOT_AN_XML_DOCUMENT, "not_an_xml_document")

    public data object InvalidXmlDocument : SqlState(INVALID_XML_DOCUMENT, "invalid_xml_document")

    public data object InvalidXmlContent : SqlState(INVALID_XML_CONTENT, "invalid_xml_content")

    public data object InvalidXmlComment : SqlState(INVALID_XML_COMMENT, "invalid_xml_comment")

    public data object InvalidXmlProcessingInstruction :
        SqlState(INVALID_XML_PROCESSING_INSTRUCTION, "invalid_xml_processing_instruction")

    public data object DuplicateJsonObjectKeyValue :
        SqlState(DUPLICATE_JSON_OBJECT_KEY_VALUE, "duplicate_json_object_key_value")

    public data object InvalidArgumentForSqlJsonDatetimeFunction :
        SqlState(
            INVALID_ARGUMENT_FOR_SQL_JSON_DATETIME_FUNCTION,
            "invalid_argument_for_sql_json_datetime_function",
        )

    public data object InvalidJsonText : SqlState(INVALID_JSON_TEXT, "invalid_json_text")

    public data object InvalidSqlJsonSubscript :
        SqlState(INVALID_SQL_JSON_SUBSCRIPT, "invalid_sql_json_subscript")

    public data object MoreThanOneSqlJsonItem :
        SqlState(MORE_THAN_ONE_SQL_JSON_ITEM, "more_than_one_sql_json_item")

    public data object NoSqlJsonItem : SqlState(NO_SQL_JSON_ITEM, "no_sql_json_item")

    public data object NonNumericSqlJsonItem :
        SqlState(NON_NUMERIC_SQL_JSON_ITEM, "non_numeric_sql_json_item")

    public data object NonUniqueKeysInAJsonObject :
        SqlState(NON_UNIQUE_KEYS_IN_A_JSON_OBJECT, "non_unique_keys_in_a_json_object")

    public data object SingletonSqlJsonItemRequired :
        SqlState(SINGLETON_SQL_JSON_ITEM_REQUIRED, "singleton_sql_json_item_required")

    public data object SqlJsonArrayNotFound :
        SqlState(SQL_JSON_ARRAY_NOT_FOUND, "sql_json_array_not_found")

    public data object SqlJsonMemberNotFound :
        SqlState(SQL_JSON_MEMBER_NOT_FOUND, "sql_json_member_not_found")

    public data object SqlJsonNumberNotFound :
        SqlState(SQL_JSON_NUMBER_NOT_FOUND, "sql_json_number_not_found")

    public data object SqlJsonObjectNotFound :
        SqlState(SQL_JSON_OBJECT_NOT_FOUND, "sql_json_object_not_found")

    public data object TooManyJsonArrayElements :
        SqlState(TOO_MANY_JSON_ARRAY_ELEMENTS, "too_many_json_array_elements")

    public data object TooManyJsonObjectMembers :
        SqlState(TOO_MANY_JSON_OBJECT_MEMBERS, "too_many_json_object_members")

    public data object SqlJsonScalarRequired :
        SqlState(SQL_JSON_SCALAR_REQUIRED, "sql_json_scalar_required")

    public data object SqlJsonItemCannotBeCastToTargetType :
        SqlState(
            SQL_JSON_ITEM_CANNOT_BE_CAST_TO_TARGET_TYPE,
            "sql_json_item_cannot_be_cast_to_target_type",
        )

    public data object IntegrityConstraintViolation :
        SqlState(INTEGRITY_CONSTRAINT_VIOLATION, "integrity_constraint_violation")

    public data object RestrictViolation : SqlState(RESTRICT_VIOLATION, "restrict_violation")

    public data object NotNullViolation : SqlState(NOT_NULL_VIOLATION, "not_null_violation")

    public data object ForeignKeyViolation :
        SqlState(FOREIGN_KEY_VIOLATION, "foreign_key_violation")

    public data object UniqueViolation : SqlState(UNIQUE_VIOLATION, "unique_violation")

    public data object CheckViolation : SqlState(CHECK_VIOLATION, "check_violation")

    public data object ExclusionViolation : SqlState(EXCLUSION_VIOLATION, "exclusion_violation")

    public data object InvalidCursorState : SqlState(INVALID_CURSOR_STATE, "invalid_cursor_state")

    public data object InvalidTransactionState :
        SqlState(INVALID_TRANSACTION_STATE, "invalid_transaction_state")

    public data object ActiveSqlTransaction :
        SqlState(ACTIVE_SQL_TRANSACTION, "active_sql_transaction")

    public data object BranchTransactionAlreadyActive :
        SqlState(BRANCH_TRANSACTION_ALREADY_ACTIVE, "branch_transaction_already_active")

    public data object HeldCursorRequiresSameIsolationLevel :
        SqlState(
            HELD_CURSOR_REQUIRES_SAME_ISOLATION_LEVEL,
            "held_cursor_requires_same_isolation_level",
        )

    public data object InappropriateAccessModeForBranchTransaction :
        SqlState(
            INAPPROPRIATE_ACCESS_MODE_FOR_BRANCH_TRANSACTION,
            "inappropriate_access_mode_for_branch_transaction",
        )

    public data object InappropriateIsolationLevelForBranchTransaction :
        SqlState(
            INAPPROPRIATE_ISOLATION_LEVEL_FOR_BRANCH_TRANSACTION,
            "inappropriate_isolation_level_for_branch_transaction",
        )

    public data object NoActiveSqlTransactionForBranchTransaction :
        SqlState(
            NO_ACTIVE_SQL_TRANSACTION_FOR_BRANCH_TRANSACTION,
            "no_active_sql_transaction_for_branch_transaction",
        )

    public data object ReadOnlySqlTransaction :
        SqlState(READ_ONLY_SQL_TRANSACTION, "read_only_sql_transaction")

    public data object SchemaAndDataStatementMixingNotSupported :
        SqlState(
            SCHEMA_AND_DATA_STATEMENT_MIXING_NOT_SUPPORTED,
            "schema_and_data_statement_mixing_not_supported",
        )

    public data object NoActiveSqlTransaction :
        SqlState(NO_ACTIVE_SQL_TRANSACTION, "no_active_sql_transaction")

    public data object InFailedSqlTransaction :
        SqlState(IN_FAILED_SQL_TRANSACTION, "in_failed_sql_transaction")

    public data object IdleInTransactionSessionTimeout :
        SqlState(IDLE_IN_TRANSACTION_SESSION_TIMEOUT, "idle_in_transaction_session_timeout")

    public data object InvalidSqlStatementName :
        SqlState(INVALID_SQL_STATEMENT_NAME, "invalid_sql_statement_name")

    public data object TriggeredDataChangeViolation :
        SqlState(TRIGGERED_DATA_CHANGE_VIOLATION, "triggered_data_change_violation")

    public data object InvalidAuthorizationSpecification :
        SqlState(INVALID_AUTHORIZATION_SPECIFICATION, "invalid_authorization_specification")

    public data object InvalidPassword : SqlState(INVALID_PASSWORD, "invalid_password")

    public data object DependentPrivilegeDescriptorsStillExist :
        SqlState(
            DEPENDENT_PRIVILEGE_DESCRIPTORS_STILL_EXIST,
            "dependent_privilege_descriptors_still_exist",
        )

    public data object DependentObjectsStillExist :
        SqlState(DEPENDENT_OBJECTS_STILL_EXIST, "dependent_objects_still_exist")

    public data object InvalidTransactionTermination :
        SqlState(INVALID_TRANSACTION_TERMINATION, "invalid_transaction_termination")

    public data object SqlRoutineException :
        SqlState(SQL_ROUTINE_EXCEPTION, "sql_routine_exception")

    public data object FunctionExecutedNoReturnStatement :
        SqlState(FUNCTION_EXECUTED_NO_RETURN_STATEMENT, "function_executed_no_return_statement")

    public data object ModifyingSqlDataNotPermitted :
        SqlState(MODIFYING_SQL_DATA_NOT_PERMITTED, "modifying_sql_data_not_permitted")

    public data object ProhibitedSqlStatementAttempted :
        SqlState(PROHIBITED_SQL_STATEMENT_ATTEMPTED, "prohibited_sql_statement_attempted")

    public data object ReadingSqlDataNotPermitted :
        SqlState(READING_SQL_DATA_NOT_PERMITTED, "reading_sql_data_not_permitted")

    public data object InvalidCursorName : SqlState(INVALID_CURSOR_NAME, "invalid_cursor_name")

    public data object ExternalRoutineException :
        SqlState(EXTERNAL_ROUTINE_EXCEPTION, "external_routine_exception")

    public data object ContainingSqlNotPermitted :
        SqlState(CONTAINING_SQL_NOT_PERMITTED, "containing_sql_not_permitted")

    public data object ModifyingSqlDataNotPermitted2 :
        SqlState(MODIFYING_SQL_DATA_NOT_PERMITTED2, "modifying_sql_data_not_permitted")

    public data object ProhibitedSqlStatementAttempted2 :
        SqlState(PROHIBITED_SQL_STATEMENT_ATTEMPTED2, "prohibited_sql_statement_attempted")

    public data object ReadingSqlDataNotPermitted2 :
        SqlState(READING_SQL_DATA_NOT_PERMITTED2, "reading_sql_data_not_permitted")

    public data object ExternalRoutineInvocationException :
        SqlState(EXTERNAL_ROUTINE_INVOCATION_EXCEPTION, "external_routine_invocation_exception")

    public data object InvalidSqlstateReturned :
        SqlState(INVALID_SQLSTATE_RETURNED, "invalid_sqlstate_returned")

    public data object NullValueNotAllowed2 :
        SqlState(NULL_VALUE_NOT_ALLOWED2, "null_value_not_allowed")

    public data object TriggerProtocolViolated :
        SqlState(TRIGGER_PROTOCOL_VIOLATED, "trigger_protocol_violated")

    public data object SrfProtocolViolated :
        SqlState(SRF_PROTOCOL_VIOLATED, "srf_protocol_violated")

    public data object EventTriggerProtocolViolated :
        SqlState(EVENT_TRIGGER_PROTOCOL_VIOLATED, "event_trigger_protocol_violated")

    public data object SavepointException : SqlState(SAVEPOINT_EXCEPTION, "savepoint_exception")

    public data object InvalidSavepointSpecification :
        SqlState(INVALID_SAVEPOINT_SPECIFICATION, "invalid_savepoint_specification")

    public data object InvalidCatalogName : SqlState(INVALID_CATALOG_NAME, "invalid_catalog_name")

    public data object InvalidSchemaName : SqlState(INVALID_SCHEMA_NAME, "invalid_schema_name")

    public data object TransactionRollback : SqlState(TRANSACTION_ROLLBACK, "transaction_rollback")

    public data object TransactionIntegrityConstraintViolation :
        SqlState(
            TRANSACTION_INTEGRITY_CONSTRAINT_VIOLATION,
            "transaction_integrity_constraint_violation",
        )

    public data object SerializationFailure :
        SqlState(SERIALIZATION_FAILURE, "serialization_failure")

    public data object StatementCompletionUnknown :
        SqlState(STATEMENT_COMPLETION_UNKNOWN, "statement_completion_unknown")

    public data object DeadlockDetected : SqlState(DEADLOCK_DETECTED, "deadlock_detected")

    public data object SyntaxErrorOrAccessRuleViolation :
        SqlState(SYNTAX_ERROR_OR_ACCESS_RULE_VIOLATION, "syntax_error_or_access_rule_violation")

    public data object SyntaxError : SqlState(SYNTAX_ERROR, "syntax_error")

    public data object InsufficientPrivilege :
        SqlState(INSUFFICIENT_PRIVILEGE, "insufficient_privilege")

    public data object CannotCoerce : SqlState(CANNOT_COERCE, "cannot_coerce")

    public data object GroupingError : SqlState(GROUPING_ERROR, "grouping_error")

    public data object WindowingError : SqlState(WINDOWING_ERROR, "windowing_error")

    public data object InvalidRecursion : SqlState(INVALID_RECURSION, "invalid_recursion")

    public data object InvalidForeignKey : SqlState(INVALID_FOREIGN_KEY, "invalid_foreign_key")

    public data object InvalidName : SqlState(INVALID_NAME, "invalid_name")

    public data object NameTooLong : SqlState(NAME_TOO_LONG, "name_too_long")

    public data object ReservedName : SqlState(RESERVED_NAME, "reserved_name")

    public data object DatatypeMismatch : SqlState(DATATYPE_MISMATCH, "datatype_mismatch")

    public data object IndeterminateDatatype :
        SqlState(INDETERMINATE_DATATYPE, "indeterminate_datatype")

    public data object CollationMismatch : SqlState(COLLATION_MISMATCH, "collation_mismatch")

    public data object IndeterminateCollation :
        SqlState(INDETERMINATE_COLLATION, "indeterminate_collation")

    public data object WrongObjectType : SqlState(WRONG_OBJECT_TYPE, "wrong_object_type")

    public data object GeneratedAlways : SqlState(GENERATED_ALWAYS, "generated_always")

    public data object UndefinedColumn : SqlState(UNDEFINED_COLUMN, "undefined_column")

    public data object UndefinedFunction : SqlState(UNDEFINED_FUNCTION, "undefined_function")

    public data object UndefinedTable : SqlState(UNDEFINED_TABLE, "undefined_table")

    public data object UndefinedParameter : SqlState(UNDEFINED_PARAMETER, "undefined_parameter")

    public data object UndefinedObject : SqlState(UNDEFINED_OBJECT, "undefined_object")

    public data object DuplicateColumn : SqlState(DUPLICATE_COLUMN, "duplicate_column")

    public data object DuplicateCursor : SqlState(DUPLICATE_CURSOR, "duplicate_cursor")

    public data object DuplicateDatabase : SqlState(DUPLICATE_DATABASE, "duplicate_database")

    public data object DuplicateFunction : SqlState(DUPLICATE_FUNCTION, "duplicate_function")

    public data object DuplicatePreparedStatement :
        SqlState(DUPLICATE_PREPARED_STATEMENT, "duplicate_prepared_statement")

    public data object DuplicateSchema : SqlState(DUPLICATE_SCHEMA, "duplicate_schema")

    public data object DuplicateTable : SqlState(DUPLICATE_TABLE, "duplicate_table")

    public data object DuplicateAlias : SqlState(DUPLICATE_ALIAS, "duplicate_alias")

    public data object DuplicateObject : SqlState(DUPLICATE_OBJECT, "duplicate_object")

    public data object AmbiguousColumn : SqlState(AMBIGUOUS_COLUMN, "ambiguous_column")

    public data object AmbiguousFunction : SqlState(AMBIGUOUS_FUNCTION, "ambiguous_function")

    public data object AmbiguousParameter : SqlState(AMBIGUOUS_PARAMETER, "ambiguous_parameter")

    public data object AmbiguousAlias : SqlState(AMBIGUOUS_ALIAS, "ambiguous_alias")

    public data object InvalidColumnReference :
        SqlState(INVALID_COLUMN_REFERENCE, "invalid_column_reference")

    public data object InvalidColumnDefinition :
        SqlState(INVALID_COLUMN_DEFINITION, "invalid_column_definition")

    public data object InvalidCursorDefinition :
        SqlState(INVALID_CURSOR_DEFINITION, "invalid_cursor_definition")

    public data object InvalidDatabaseDefinition :
        SqlState(INVALID_DATABASE_DEFINITION, "invalid_database_definition")

    public data object InvalidFunctionDefinition :
        SqlState(INVALID_FUNCTION_DEFINITION, "invalid_function_definition")

    public data object InvalidPreparedStatementDefinition :
        SqlState(INVALID_PREPARED_STATEMENT_DEFINITION, "invalid_prepared_statement_definition")

    public data object InvalidSchemaDefinition :
        SqlState(INVALID_SCHEMA_DEFINITION, "invalid_schema_definition")

    public data object InvalidTableDefinition :
        SqlState(INVALID_TABLE_DEFINITION, "invalid_table_definition")

    public data object InvalidObjectDefinition :
        SqlState(INVALID_OBJECT_DEFINITION, "invalid_object_definition")

    public data object WithCheckOptionViolation :
        SqlState(WITH_CHECK_OPTION_VIOLATION, "with_check_option_violation")

    public data object InsufficientResources :
        SqlState(INSUFFICIENT_RESOURCES, "insufficient_resources")

    public data object DiskFull : SqlState(DISK_FULL, "disk_full")

    public data object OutOfMemory : SqlState(OUT_OF_MEMORY, "out_of_memory")

    public data object TooManyConnections : SqlState(TOO_MANY_CONNECTIONS, "too_many_connections")

    public data object ConfigurationLimitExceeded :
        SqlState(CONFIGURATION_LIMIT_EXCEEDED, "configuration_limit_exceeded")

    public data object ProgramLimitExceeded :
        SqlState(PROGRAM_LIMIT_EXCEEDED, "program_limit_exceeded")

    public data object StatementTooComplex :
        SqlState(STATEMENT_TOO_COMPLEX, "statement_too_complex")

    public data object TooManyColumns : SqlState(TOO_MANY_COLUMNS, "too_many_columns")

    public data object TooManyArguments : SqlState(TOO_MANY_ARGUMENTS, "too_many_arguments")

    public data object ObjectNotInPrerequisiteState :
        SqlState(OBJECT_NOT_IN_PREREQUISITE_STATE, "object_not_in_prerequisite_state")

    public data object ObjectInUse : SqlState(OBJECT_IN_USE, "object_in_use")

    public data object CantChangeRuntimeParam :
        SqlState(CANT_CHANGE_RUNTIME_PARAM, "cant_change_runtime_param")

    public data object LockNotAvailable : SqlState(LOCK_NOT_AVAILABLE, "lock_not_available")

    public data object UnsafeNewEnumValueUsage :
        SqlState(UNSAFE_NEW_ENUM_VALUE_USAGE, "unsafe_new_enum_value_usage")

    public data object OperatorIntervention :
        SqlState(OPERATOR_INTERVENTION, "operator_intervention")

    public data object QueryCanceled : SqlState(QUERY_CANCELED, "query_canceled")

    public data object AdminShutdown : SqlState(ADMIN_SHUTDOWN, "admin_shutdown")

    public data object CrashShutdown : SqlState(CRASH_SHUTDOWN, "crash_shutdown")

    public data object CannotConnectNow : SqlState(CANNOT_CONNECT_NOW, "cannot_connect_now")

    public data object DatabaseDropped : SqlState(DATABASE_DROPPED, "database_dropped")

    public data object IdleSessionTimeout : SqlState(IDLE_SESSION_TIMEOUT, "idle_session_timeout")

    public data object SystemError : SqlState(SYSTEM_ERROR, "system_error")

    public data object IoError : SqlState(IO_ERROR, "io_error")

    public data object UndefinedFile : SqlState(UNDEFINED_FILE, "undefined_file")

    public data object DuplicateFile : SqlState(DUPLICATE_FILE, "duplicate_file")

    public data object SnapshotTooOld : SqlState(SNAPSHOT_TOO_OLD, "snapshot_too_old")

    public class Unknown(code: String) : SqlState(code, "Unknown code") {
