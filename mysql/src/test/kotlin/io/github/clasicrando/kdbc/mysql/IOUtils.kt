...
object IOUtils {
    ...
    /**
     * Open the [path] as a writable [kotlinx.io.RawSink]. If [append] is true, all contents written to the
     * [kotlinx.io.RawSink] are added to the end of the file.
     */
    fun sink(path: Path, append: Boolean = false): RawSink =
        SystemFileSystem.sink(path = path, append = append)

    /** Open the [path] as a readable [kotlinx.io.RawSource] */
